...
export class Events {
  ...
  consumeFirst(types: string[], calledFrom?: GameMethod) {
    const index = this.dispatch.findIndex((event) => {
      if (!types.includes(event.type)) return false;
      return calledFrom === undefined || event.calledFrom === calledFrom;
    });
    if (index === -1) return;
    const [event] = this.dispatch.splice(index, 1);
    return { type: event.type, args: event.args };
  }
