...
const PluginActionNotFound = 'plugin action not found';
const PluginActionReturnedPromise =
...
function clonePluginData<Data>(data: Data): Data {
  return structuredClone(data);
}

function PluginActionError(state: State, plugin: string, message: string): PluginActionResult {
  error(`${plugin} plugin action failed:\n${message}`);
  return { state, error: { plugin, message } };
}
 ...
 *
 * Failure contract:
 * - an unknown plugin action leaves state unchanged and returns an error;
 * - a throwing plugin action leaves state unchanged and returns an error;
 * - a Promise-like return is rejected because plugin actions are synchronous.
 */
export const ProcessAction = (
  ...
  const matchingPlugins = opts.game.plugins
    .filter((plugin) => plugin.action !== undefined)
    .filter((plugin) => plugin.name === action.payload.type);

  if (matchingPlugins.length === 0) {
    return PluginActionError(state, action.payload.type, PluginActionNotFound);
  }
  ...
  for (const plugin of matchingPlugins) {
    ...
    const pluginState = nextState.plugins[name] || { data: {} };
    let data: unknown;
    try {
      data = actionFn(clonePluginData(pluginState.data), action.payload);
    } catch (caught) {
      return PluginActionError(state, name, `plugin action threw: ${errorMessage(caught)}`);
    }

    if (isPromiseLike(data)) {
      return PluginActionError(state, name, PluginActionReturnedPromise);
    }
 ...
 */
export const FnWrap = (methodToWrap: AnyFn, methodType: GameMethod, plugins: Plugin[]) => {
  return [...CORE_PLUGINS, ...plugins, PluginEvents]
    .filter((plugin) => plugin.fnWrap !== undefined)
    .reduce((method: AnyFn, { fnWrap }: Plugin) => fnWrap(method, methodType), methodToWrap);
};

/**
 * Allows the plugin to generate its initial state.
 */
export const Setup = (state: PartialGameState, opts: PluginOpts): PartialGameState => {
...
const Flush = (state: State, opts: PluginOpts): State => {
  ...
  [...CORE_PLUGINS, ...opts.game.plugins, PluginEvents].reverse().forEach((plugin) => {
    const name = plugin.name;
    const pluginState = state.plugins[name] || { data: {} };
 ...
 */
const IsInvalid = (state: State, opts: PluginOpts): false | { plugin: string; message: string } => {
