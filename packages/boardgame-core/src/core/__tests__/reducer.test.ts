...
test('sync', () => {
  const state = reducer(undefined, sync({ state: { G: 'restored' } } as SyncInfo));
...
describe('Plugin Action Error API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  test('invalid plugin name returns a transient error and keeps state unchanged', () => {
    const game: Game = {
      plugins: [
        {
          name: 'counter',
          setup: () => ({ count: 0 }),
          action: (data) => ({ count: data.count + 1 }),
        },
      ],
    };
    const reducer = CreateGameReducer({ game });
    ...
    const result = reducer(state, pluginAction('missing', undefined, '0'));

    expect(result.G).toEqual(state.G);
    expect(result.plugins).toEqual(state.plugins);
    expect(result['transients'].error).toEqual({
      type: 'action/plugin_invalid',
      payload: {
        plugin: 'missing',
    ...
    expect(error).toBeCalledWith('missing plugin action failed:\nplugin action not found');
  });

  test('plugin action throw returns a transient error and keeps state unchanged', () => {
    const game: Game = {
      plugins: [
        {
          name: 'exploder',
    ...
    const result = reducer(state, pluginAction('exploder', undefined, '0'));
    ...
    expect(result['transients'].error).toEqual({
      ...
      payload: {
        plugin: 'exploder',
        message: 'plugin action threw: boom',
      },
    });
    expect(error).toBeCalledWith('exploder plugin action failed:\nplugin action threw: boom');
  });

  test('plugin action throw after data mutation keeps state unchanged', () => {
    const game: Game = {
      plugins: [
        {
          name: 'mutating-exploder',
          setup: () => ({ nested: { stable: true } }),
          action: (data) => {
            data.nested.stable = false;
            throw new Error('boom');
          },
        },
      ],
    ...
    const result = reducer(state, pluginAction('mutating-exploder', undefined, '0'));

    expect(state.plugins['mutating-exploder'].data).toEqual({ nested: { stable: true } });
    expect(result.plugins).toEqual(state.plugins);
    expect(result['transients'].error).toEqual({
      type: 'action/plugin_invalid',
      payload: {
        plugin: 'mutating-exploder',
        message: 'plugin action threw: boom',
      },
    });
  });

  test('Promise-like plugin action return is invalid and keeps state unchanged', () => {
    const game: Game = {
      plugins: [
        {
          name: 'async',
          setup: () => ({ stable: true }),
          action: () => Promise.resolve({ stable: false }),
        },
      ],
    };
    const reducer = CreateGameReducer({ game });
    ...
    const result = reducer(state, pluginAction('async', undefined, '0'));
    ...
    expect(result['transients'].error).toEqual({
      ...
      payload: {
        plugin: 'async',
        message: 'plugin action returned a Promise; plugin actions must be synchronous',
      },
    });
    expect(error).toBeCalledWith(
      'async plugin action failed:\nplugin action returned a Promise; plugin actions must be synchronous',
    );
  });

  test('Promise-like plugin action return after data mutation keeps state unchanged', () => {
    const game: Game = {
      plugins: [
        {
          name: 'mutating-async',
    ...
    const result = reducer(state, pluginAction('mutating-async', undefined, '0'));

    expect(state.plugins['mutating-async'].data).toEqual({ nested: { stable: true } });
    expect(result.plugins).toEqual(state.plugins);
    expect(result['transients'].error).toEqual({
      type: 'action/plugin_invalid',
      payload: {
        plugin: 'mutating-async',
        message: 'plugin action returned a Promise; plugin actions must be synchronous',
      },
    });
  });
...
describe('undo / redo', () => {
  ...
  test('undo restores previous state after event', () => {
    const initial = reducer(initialState, gameEvent('setStage', 'special', '0'));
...
describe('TransientHandlingMiddleware', () => {
  ...
  test('regular dispatch result has no transients', () => {
    ...
    expect(result).toEqual(expect.not.objectContaining({ transients: expect.anything() }));
