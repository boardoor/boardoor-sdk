...
import { UpdateTurnOrderState, Stage, TurnOrder, ActivePlayers } from '../turn-order';
...
describe('setActivePlayers', () => {
  ...
  test('short form', () => {
    const newState = flow.processEvent(state, gameEvent('setActivePlayers', [['1', '2']]));
  ...
  test('all', () => {
    const newState = flow.processEvent(state, gameEvent('setActivePlayers', [{ all: Stage.NULL }]));
    expect(newState.ctx.activePlayers).toMatchObject({
      '0': Stage.NULL,
      '1': Stage.NULL,
    });
...
describe('UpdateTurnOrderState', () => {
  ...
  test('without next player', () => {
    const { ctx: t } = UpdateTurnOrderState({ G, ctx } as State, ctx.currentPlayer, turn);
