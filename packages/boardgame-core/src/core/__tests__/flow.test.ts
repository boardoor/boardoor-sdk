...
describe('stage events', () => {
  describe('setStage', () => {
    ...
    test('with multiple active players', () => {
      ...
      state = flow.processEvent(state, gameEvent('setStage', { stage: 'B', minMoves: 1 }));
      expect(state.ctx.activePlayers).toEqual({ '0': 'B', '1': 'A', '2': 'A' });

      state = flow.processEvent(state, gameEvent('setStage', { stage: 'B', maxMoves: 1 }, '1'));
    ...
    test('with min moves', () => {
      ...
      state = flow.processEvent(state, gameEvent('setStage', { stage: 'A', minMoves: 1 }));
    ...
    test('with max moves', () => {
      ...
      expect(state.ctx._activePlayersMinMoves).toBeNull();
      expect(state.ctx._activePlayersMaxMoves).toBeNull();
      state = flow.processEvent(state, gameEvent('setStage', { stage: 'A', maxMoves: 1 }));
      expect(state.ctx._activePlayersMinMoves).toBeNull();
    ...
    describe('disallowed in hooks', () => {
      ...
      test('phase.onBegin', () => {
        ...
        const errorMessage = (error as ReturnType<typeof vi.fn>).mock.calls[0][0];
...
describe('endIf', () => {
  ...
  test('phase automatically ends', () => {
    ...
    expect(
      client.getState().deltalog[client.getState().deltalog.length - 1].action.payload.type,
    ).toBe('endPhase');
  });
...
test('isPlayerActive', () => {
  ...
  expect(flow.isPlayerActive({}, { currentPlayer: '0' } as Ctx, playerID)).toBe(true);
});
...
describe('infinite loops', () => {
  ...
  test('endIf that triggers endIf', () => {
    ...
    const errorMessage = (error as ReturnType<typeof vi.fn>).mock.calls[0][0];
    expect(errorMessage).toMatch(/events plugin declared action invalid/);
    expect(errorMessage).toMatch(/Events must be called from moves or the `.+` hooks./);
  });
});
...
describe('events in hooks', () => {
  ...
  describe('endPhase', () => {
    ...
    test('can end phase from turn.onEnd', () => {
      const endPhaseFromTurnEnd = ({ G, events }) => {
        G.turnEndCount = (G.turnEndCount || 0) + 1;
        if (!G.shouldEnd) return;
        G.shouldEnd = false;
        events.endPhase();
      };
      const client = Client({
        game: {
          ...
          phases: {
            A: {
              start: true,
              turn: { onEnd: endPhaseFromTurnEnd },
            },
          },
        },
      });
      ...
      client.events.endTurn();
      state = client.getState();
      expect(state.G.turnEndCount).toBe(1);
      expect(state.ctx.turn).toBe(2);
      expect(state.ctx.currentPlayer).toBe('1');
      expect(state.ctx.phase).toBeNull();
    });

    test('can set phase from turn.onEnd without skipping a turn', () => {
      const setPhaseFromTurnEnd = ({ G, events }) => {
        G.turnEndCount = (G.turnEndCount || 0) + 1;
        if (!G.shouldEnd) return;
        G.shouldEnd = false;
        events.setPhase('B');
      ...
      const client = Client({
        game: {
          ...
          phases: {
            A: {
              start: true,
              turn: { onEnd: setPhaseFromTurnEnd },
            },
            B: {},
          },
        },
    ...
    test('can end game from turn.onEnd without starting a replacement turn', () => {
      const endGameFromTurnEnd = ({ G, events }) => {
        G.turnEndCount = (G.turnEndCount || 0) + 1;
        if (!G.shouldEnd) return;
        G.shouldEnd = false;
...
describe('backwards compatibility for moveLimit', () => {
  ...
  test('setActivePlayers maps moveLimit to maxMoves only', () => {
    ...
    state = flow.processEvent(state, gameEvent('setActivePlayers', { all: 'A', moveLimit: 1 }));
  ...
  test('setStage maps moveLimit to maxMoves only', () => {
    ...
    expect(state.ctx._activePlayersMinMoves).toBeNull();
    expect(state.ctx._activePlayersMaxMoves).toBeNull();
    state = flow.processEvent(state, gameEvent('setStage', { stage: 'A', moveLimit: 2 }));
    expect(state.ctx._activePlayersMinMoves).toBeNull();
...
describe('hook execution order', () => {
  ...
  test('hooks called on move', () => {
    client.moves.move();
    expect(calls).toEqual(['move', 'turn.onMove', 'game.endIf', 'phaseA.endIf', 'turn.endIf']);
  });
...
describe('game function signatures', () => {
  ...
  const FnContext = ({ playerID, G = 'G' }: { playerID?: PlayerID; G?: any } = {}) => {
    const context: any = {
      G,
      ctx: expectCtx,
      events: expectEvents,
  ...
  beforeEach(() => {
    game = {
      ...
      turn: {
        order: {
          playOrder: vi.fn(({ ctx }) => Array.from({ length: ctx.numPlayers }, (_, i) => i + '')),
          first: vi.fn(TurnOrder.DEFAULT.first),
          next: vi.fn(TurnOrder.DEFAULT.next),
        },
        onBegin: vi.fn(),
        onMove: vi.fn(),
        onEnd: vi.fn(),
