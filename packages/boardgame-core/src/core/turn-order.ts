...
export function UpdateActivePlayersOnceEmpty(ctx: Ctx) {
  ...
  if (activePlayers && Object.keys(activePlayers).length === 0) {
    if (_nextActivePlayers) {
      ...
      ({ activePlayers, _activePlayersMinMoves, _activePlayersMaxMoves, _activePlayersNumMoves } =
 ...
 */
function getCurrentPlayer(playOrder: Ctx['playOrder'], playOrderPos: Ctx['playOrderPos']) {
...
export function InitTurnOrderState(state: State, turn: TurnConfig) {
  ...
  let playOrder = Array.from({ length: numPlayers }, (_, i) => i + '');
...
export const TurnOrder = {
  ...
  DEFAULT: {
    first: ({ ctx }: FnContext) =>
      ctx.turn === 0 ? ctx.playOrderPos : (ctx.playOrderPos + 1) % ctx.playOrder.length,
