 ...
 */
export function Flow({ moves, phases, endIf, onEnd, turn, events, plugins }: Game) {
  // Attach defaults.
  if (moves === undefined) {
    moves = {};
  }
  ...
  const HookWrapper = (hook: (context: FnContext) => any, hookType: GameMethod) => {
  ...
  function ConsumeQueuedTurnEndTransition(state: State) {
    const events = state.plugins?.events?.api as
      | {
          _private?: {
            consumeFirst?: (
              types: string[],
              calledFrom?: GameMethod,
            ) => { type: string; args: any[] } | undefined;
          };
        }
      | undefined;
    return events?._private?.consumeFirst?.(
      ['endPhase', 'setPhase', 'endGame'],
      GameMethod.TURN_ON_END,
    );
  }

  function QueueTurnEndTransition(
    next: {
      fn: (state: State, opts: any) => State;
      arg?: any;
      turn?: Ctx['turn'];
    ...
    event: { type: string; args: any[] },
    { turn, phase }: { turn: Ctx['turn']; phase: Ctx['phase'] },
  ) {
    switch (event.type) {
      case 'endPhase':
        next.push({ fn: EndPhase, turn, phase, skipTurnEnd: true });
        break;
      case 'setPhase':
        next.push({ fn: EndPhase, arg: { next: event.args[0] }, turn, phase, skipTurnEnd: true });
        break;
      case 'endGame':
        next.push({ fn: EndGame, arg: event.args[0], turn, phase, skipTurnEnd: true });
        break;
    }
  }
  ...
  function UpdateStage(state: State, { arg, playerID }): State {
    ...
    let { ctx } = state;
    let { activePlayers, _activePlayersMinMoves, _activePlayersMaxMoves, _activePlayersNumMoves } =
      ctx;
  ...
  function EndGame(state: State, { arg, phase, skipTurnEnd }): State {
    state = EndPhase(state, { phase, skipTurnEnd });
  ...
  function EndPhase(
    state: State,
    { arg, next, turn: initialTurn, automatic, skipTurnEnd }: any,
  ): State {
    // End the turn first.
    if (!skipTurnEnd) {
  ...
  function EndTurn(
    ...
    if (!force && phaseConfig.turn.minMoves && currentPlayerMoves < phaseConfig.turn.minMoves) {
      logging.info(`cannot end turn before making ${phaseConfig.turn.minMoves} moves`);
      return state;
    }

    // Run turn-end triggers.
    const G = phaseConfig.turn.wrapped.onEnd(state);
    // Phase/game transitions queued by turn.onEnd belong to the turn that just ended.
    // Process them before starting a replacement turn so the replacement is not skipped.
    const queuedTurnEndEvent = next ? ConsumeQueuedTurnEndTransition(state) : undefined;

    if (next) {
      if (queuedTurnEndEvent) {
        QueueTurnEndTransition(next, queuedTurnEndEvent, { turn, phase });
      } else {
  ...
  function EndStage(state: State, { arg, next, automatic, playerID }: any): State {
    ...
    if (
      ...
      logging.info(`cannot end stage before making ${_activePlayersMinMoves[playerID]} moves`);
      return state;
    }
  ...
  function SetActivePlayersEvent(state: State, _playerID: PlayerID, arg: ActivePlayersArg): State {
    return Process(state, [{ fn: UpdateActivePlayers, arg }]);
  }

  function SetPhaseEvent(state: State, _playerID: PlayerID, newPhase: string): State {
  ...
  function EndPhaseEvent(state: State): State {
    return Process(state, [{ fn: EndPhase, phase: state.ctx.phase, turn: state.ctx.turn }]);
  }

  function EndTurnEvent(state: State, _playerID: PlayerID, arg: any): State {
    return Process(state, [{ fn: EndTurn, turn: state.ctx.turn, phase: state.ctx.phase, arg }]);
  }
  ...
  function EndGameEvent(state: State, _playerID: PlayerID, arg: any): State {
    return Process(state, [{ fn: EndGame, turn: state.ctx.turn, phase: state.ctx.phase, arg }]);
  }
  ...
  return {
    ctx: (numPlayers: number): Ctx => ({
      ...
      turn: 0,
      currentPlayer: '0',
      playOrder: Array.from({ length: numPlayers }, (_, i) => i + ''),
      playOrderPos: 0,
      phase: startingPhase,
      activePlayers: null,
    }),
