...
export const TransientHandlingMiddleware =
  (store: Store) => (next: Dispatch<ActionShape.Any>) => (action: ActionShape.Any) => {
 ...
 */
export function CreateGameReducer({ game, isClient }: { game: Game; isClient?: boolean }) {
  ...
  return (
    ...
    switch (action.type) {
      ...
      case Actions.UNDO: {
        ...
        if (last.moveType) {
          const lastMove: Move = game.flow.getMove(restore.ctx, last.moveType, last.playerID);
      ...
      case Actions.PLUGIN: {
        const result = plugins.ProcessAction(state, action, { game });
        if (result.error) {
          return WithError(result.state, ActionErrorType.PluginActionInvalid, result.error);
        }
        return result.state;
      }
