...
export interface PluginActionErrorPayload {
  plugin: string;
  message: string;
}

export type PluginActionResult<G extends any = any> = {
  state: State<G>;
  error?: PluginActionErrorPayload;
};
...
export type ActionResult<G extends any = any> = TransientState<G>;
...
export interface Ctx {
  ...
  turn: number;
  phase: string;
  _activePlayersMinMoves?: Record<PlayerID, number> | null;
  _activePlayersMaxMoves?: Record<PlayerID, number> | null;
  ...
  _nextActivePlayers?: ActivePlayersArg | null;
  _random?: {
    seed: string | number;
  };
}
...
export interface Plugin<API extends any = any, Data extends any = any, G extends any = any> {
  ...
  api?: (context: { G: G; ctx: Ctx; game: Game<G>; data: Data; playerID?: PlayerID }) => API;
...
> {
  ...
  endIf?: (context: FnContext<G, PluginAPIs>) => boolean | void | { next: string };
...
> {
  ...
  onBegin?: (context: FnContext<G, PluginAPIs>) => void | G;
  onEnd?: (context: FnContext<G, PluginAPIs>) => void | G;
  endIf?: (context: FnContext<G, PluginAPIs>) => boolean | void | { next: PlayerID };
  onMove?: (context: FnContext<G, PluginAPIs> & { playerID: PlayerID }) => void | G;
...
export type AiEnumerate = Array<
  | { event: string; args?: any[]; weight?: number }
  | { move: string; args?: any[]; weight?: number }
...
/** Metadata exposed to an internal, experimental game-state migration. */
export interface GameMigrationMeta {
  fromVersion: number;
  toVersion: number;
  numPlayers: number;
  slot: 'state' | 'initialState' | 'undo' | 'redo';
}

/**
 * Pure, synchronous migration for game-owned G only.
 *
 * This contract is internal / experimental and is not part of the public SDK
 * stability surface yet.
 */
export type GameStateMigration<GFrom = unknown, GTo = unknown> = (
  G: GFrom,
  meta: GameMigrationMeta,
) => GTo;

/** Internal, experimental schema declaration for game-owned state. */
export interface GameStateSchema {
  version: number;
  migrations: Readonly<Record<number, GameStateMigration>>;
}

/**
 * Optional player-specific projections for transport metadata that lives
 * outside G. These functions must be pure and must not mutate their inputs.
 */
export interface TransportView<G extends any = any> {
  ctx?: (context: { G: G; ctx: Ctx; playerID: PlayerID | null }) => Ctx;
  log?: (context: {
    G: G;
    ctx: Ctx;
    playerID: PlayerID | null;
...
> {
  ...
  disableUndo?: boolean;
  seed?: string | number;
  setup?: (context: PluginAPIs & DefaultPluginAPIs & { ctx: Ctx }, setupData?: SetupData) => G;
  ...
  transportView?: TransportView<G>;
  stateSchema?: GameStateSchema;
  computeShareResult?: (context: { G: G; ctx: Ctx; playerID: PlayerID }) => ShareResult;
  ...
  ai?: {
    ...
    sampleHiddenState?: (G: G, playerID: PlayerID) => G;
    domainInvariants?: (G: G, ctx: Ctx, meta: DomainInvariantContext) => DomainInvariantViolation[];
    /** Optional async initializer called once before the first bestMove invocation (e.g. load NN weights). */
    init?: () => Promise<void>;
    /** Optional direct evaluator that bypasses MCTS. Returns the best move or null. May be async. */
    bestMove?: (
      G: G,
      ctx: Ctx,
      playerID: PlayerID,
      strength?: 'medium' | 'strong',
    ) => { move: string; args: any[] } | null | Promise<{ move: string; args: any[] } | null>;
    /** Optional per-action pacing override for AI moves. Defaults to 600ms when omitted. */
    getMoveDelayMs?: (context: {
      G: G;
      ctx: Ctx;
      playerID: PlayerID;
...
export namespace Server {
  export type GenerateCredentials = (ctx: any) => Promise<string> | string;
...
export namespace CredentialedActionShape {
  ...
  export type Any = MakeMove | GameEvent | AutomaticGameEvent | Undo | Redo | Plugin;
}

type StripPayloadCredentials<T> = T extends { payload: infer P }
  ? Omit<T, 'payload'> & { payload: Omit<P, 'credentials'> }
  : T;

export namespace ActionShape {
  type StripCredentials<T extends CredentialedActionShape.Any> = StripPayloadCredentials<T>;
