...
type DebugConstructor = new (options: {
  target: HTMLElement;
...
export class ClientManager {
  ...
  private defaultDebug: DebugConstructor;

  constructor(defaultDebug: DebugConstructor = Debug) {
  ...
  private mountDebug(client: _ClientImpl): void {
    ...
    let DebugImpl: DebugConstructor | undefined;
    let target = document.body;

    // Preserve boardgame.io's production default while allowing an explicit
    // debug option to opt into Boardoor's vendored panel in production.
    if (process.env.NODE_ENV !== 'production' || client.debugOpt !== undefined) {
      DebugImpl = this.defaultDebug;
    }
