...
import type { TransportData } from '../core/transport-data';
...
type ClientAction = ActionShape.Reset | ActionShape.Sync | ActionShape.Update | ActionShape.Patch;
type Action = CredentialedActionShape.Any | ActionShape.StripTransients | ClientAction;
type StoreActionType = 'makeMove' | 'gameEvent' | 'plugin';
type StoreActionCreator =
  | typeof ActionCreators.makeMove
  | typeof ActionCreators.gameEvent
  | typeof ActionCreators.plugin;
...
function createDispatchers(
  ...
  const createStoreAction = getStoreActionCreator(storeActionType);
...
function getStoreActionCreator(storeActionType: StoreActionType): StoreActionCreator {
  switch (storeActionType) {
    case 'makeMove':
      return ActionCreators.makeMove;
    case 'gameEvent':
      return ActionCreators.gameEvent;
    case 'plugin':
      return ActionCreators.plugin;
  }
}
...
> {
  ...
  constructor({
     ...
     */
    const LogMiddleware = (store: Store) => (next: Dispatch<Action>) => (action: Action) => {
     ...
     */
    const TransportMiddleware = (store: Store) => (next: Dispatch<Action>) => (action: Action) => {
      const baseState = store.getState();
      const result = next(action);

      if (!('clientOnly' in action) && action.type !== Actions.STRIP_TRANSIENTS) {
     ...
     */
    const SubscriptionMiddleware = () => (next: Dispatch<Action>) => (action: Action) => {
      const result = next(action);
    ...
    enhancer =
      enhancer !== undefined
        ? (compose(middleware, enhancer) as unknown as StoreEnhancer)
        : middleware;
  ...
  private receiveTransportData(data: TransportData): void {
    ...
    switch (data.type) {
      ...
      case 'patch': {
        ...
        const action = ActionCreators.patch(prevStateID, stateID, patch, deltalog);
        this.store.dispatch(action);
